...
def test_vec3(vec_txt_file, write_flag=0):
    if write_flag == 1:
        vec_dict = {}
        tfile = open(vec_txt_file)
        while 1:
        ...
        write_txt_vec(vec_dict, vec_txt_file + '.bin')
        vec_load = load_txt_vec(vec_txt_file + '.bin')
    else:
        vec_load = load_txt_vec(vec_txt_file)
        for vname in vec_load:
            s = vname
